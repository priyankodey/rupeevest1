class BanksController < ApplicationController
	...
	def create
	    ...
	    @bank.user_id = current_user.id
	  	if @bank.name_of_bank == "Kotak Bank"
			...
			@bank.bank_sip_mandate_status = 0

		if @bank.save
		  	
		  @bank.umrn = @bank.umrn_no
		  @bank.save	
  	...
  	def mandate
		...
		@banks = Bank.where(:bank_sip_mandate_status => 0)
  ...
  private
	 ...
	 :bank_first_holder,:bank_branch_address, :bank_city, :bank_ifsc_code, :bank_micr_code,:bank_second_holder,
	  :bank_sip_mandate_status, :bank_sip_year, :bank_sip_mandate, :umrn,:reason, :bankid_pg)
